...
#' Ratio estimation.
#' @param num
#'   A numeric vector, the numerator of the ratio.
#' @param den
#'   A numeric vector, the denominator of the ratio.
#' @param adj
#'   A numeric scalar, an adjustment factor to be multiplied by the ratio,
#'   default 1.
#' @details
#'   All records with a missing value in either the numerator or
#'   the denominator are omitted from calculations.
#' @return
#'   A named vector with the sum of the numerators, the sum of the denominators,
#'   the ratio of the sums,	the number of non-missing input pairs, and
#'   the standard deviation and 95% confidence interval of the ratio.
#' @export
#' @seealso
#'   \code{\link[survey]{svydesign}}, \code{\link[survey]{svyratio}}.
#' @import
#'   survey
#' @examples
...
ratest <- function(num, den, adj=1) {
	...
	if (n < 2) {
		...
		mydesign <- svydesign(ids=~ 1, weights=1, data=data.frame(n=num[sel],
      d=den[sel]))
		myratio <- svyratio(~n, ~d, mydesign)
		um <- as.vector(unlist(myratio))
		ci <- as.vector(confint(myratio)[1, ])
		ci[1][ci[1]<0] <- 0
		out <- c(sumnum=sumnum, sumden=sumden, rat=rat, n=n, sd=adj*sqrt(um[2]),
      cilo=adj*ci[1], cihi=adj*ci[2])
	}
	out
}
