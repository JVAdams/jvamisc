...
#' @param pos
#'   A scalar of the environment, search path index, or package of interest,
#'   default 1.
#' @param pat
#'   Regular expression pattern specifying which members to return,
...
#' @export
#' @references
#'   Based on a method posted by Bendix Carstensen on 10 Jan 2007 on R-help
...
lls <- function(pos=1, pat="") {
	...
	if (length(lll) > 0) {
		x <- data.frame(array(dim=c(length(lll), 3),
      dimnames=list(lll, c("class", "ndim", "size"))))
		y <- rep(NA, length(lll))
		for(i in 1:length(lll)) {
			x$class[i] <- paste(eval(parse(text=paste("class(", lll[i], ")"))),
        collapse=" ")
			x$ndim[i] <- length(eval(parse(text=paste("dimx(", lll[i], ")"))))
			x$size[i] <- paste(eval(parse(text=paste("dimx(", lll[i], ")"))),
        collapse=" x ")
