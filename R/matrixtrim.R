#' Trim Matrix
#'
#' Trim the rows and columns of a logical matrix until every row and columns
#' has a specified level of TRUE values.
#' @param m
#'   A logical matrix to be trimmed, should be all TRUE, FALSE (or 0, 1).
#' @param prop
#'   A numeric vector of length two, the minimum proportion of TRUE values in
#'   each row and column of the trimmed matrix,
#' default c(1, 1).
#' @param rowsfirst
#'   A logical scalar, indicating if rows (TRUE, the default) or
#'   columns (FALSE) should be trimmed from the matrix first.
#' @return
#'   A list of length three:
#'   \itemize{
#'     \item \code{trim}=a logical matrix, the resulting trimmed matrix
#'     \item \code{dim}=a numeric vector of length 2, the dimensions of
#'       \code{trim}
#'     \item \code{n}=a numeric scalar, the total number of TRUE values in
#'       \code{trim}
#'   }
#' @export
#' @examples
