...
#' @param y
#'   A numeric vector, the second set of data to be binned into a histogram,
#'   default NULL.
#' @param type
#'   A character scalar, indicating to which plot axes histograms should be
#'   added; \code{"x"} adds a histogram along the x-axis,
#'   \code{"y"} adds a histogram along the y-axis,
#'   \code{"xy"} (the default) adds a histogram along both axes.
#' @param nclass
#'   An integer scalar, the target number of bins for the histogram, default 20.
#' @param newmar
#'   A numeric vector of length 2, indicating new margins to use,
#'   default c(0, 1).
#' @param adj.fac
#'   A numeric scalar, adjustment factor for extent of y-axis,
#'   default 1.05 (to ensure tallest bars in histogram are below axis box).
#' @param xlab
#'   A character scalar, label for x-axis, default "Frequency".
#' @param ylab
#'   A character scalar, label for y-axis, default "".
#' @param fill
#'   A character or numeric scalar, color for filling in histogram bars,
#'   default "gray".
#' @export
#' @seealso
#'   \code{\link{hist}}.
