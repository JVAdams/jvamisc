...
#' @param file 		Character scalar name of document, default "RGeneratedDocument" with \code{\link{Sys.Date}} suffix.
#' @param dir 		Character scalar name of directory where document should be stored, default \code{\link{getwd}()}.
#' @param width 	Numeric scalar width of document page in inches, default 8.5.
#' @param height 	Numeric scalar height of document page in inches, default 11.
#' @param omi 		Numeric vector, length 4, width of document page margins in inches (bottom, left, top, right), default c(1, 1, 1, 1).
#' @param quiet 	Logical scalar indicating if name of new rtf document should be printed to command line, default FALSE.
#' @return 			An rtf file is created in the specified directory.  
#' An object of class rtf is created.  This object is referred to in other function to write to the file.
#' In addition, two numeric vectors of length 1, \code{tabcount} and \code{figcount}, are written to the working directory to keep track
#' of the number of tables and figures written to the rtf document, and label the captions accordingly.
